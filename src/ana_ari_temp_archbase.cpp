...
ana_ari_temp_archbase::ana_ari_temp_archbase(TString fileList) : fChain(0) 
{
  ifstream indata;
...
ana_ari_temp_archbase::ana_ari_temp_archbase(TString inFileName, Int_t keyID) : fChain(0) 
{
  if(keyID == 1){
    ifstream indata;
...
void ana_ari_temp_archbase::tGraphInit(TGraph *gr[nChannels], TString grName, TString grTitle){
  Int_t i;
  TString grNameh;
  TString grTitleh;
...
void ana_ari_temp_archbase::h1D1Init(TH1D *h1D1[nChannels],TString h1name, TString h1Title,
...
void ana_ari_temp_archbase::h2D2Init(TH2D *h2D1[nChannels],TString h2name, TString h2Title,
...
void ana_ari_temp_archbase::tProfInit(TProfile *tprof[nChannels],TString prname, TString prTitle,
		       Int_t Nbin, Float_t Vmin, Float_t Vmax){
  Int_t i;
  TString prnameh;
  TString prTitleh;
...
double ana_ari_temp_archbase::getUnixTimeFromTime(double d_year, double d_month, double d_day, double d_hour, double d_min, double d_sec){
  //time_t timer;
  struct tm y2k = {0};
...
ana_ari_temp_archbase::~ana_ari_temp_archbase(){
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

void ana_ari_temp_archbase::Loop(TString histOut){
}

Int_t ana_ari_temp_archbase::GetEntry(Long64_t entry){
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}

Long64_t ana_ari_temp_archbase::LoadTree(Long64_t entry){
...
void ana_ari_temp_archbase::Init(TTree *tree){
...
Bool_t ana_ari_temp_archbase::Notify(){
  // The Notify() function is called when a new file is opened. This
  // can be either for a new TTree in a TChain or when when a new TTree
  // is started when using PROOF. It is normally not necessary to make changes
  // to the generated code, but the routine can be extended by the
  // user if needed. The return value is currently not used.
  //cout<<"Notify() --> nData = "<<nData<<endl;
  return kTRUE;
}

void ana_ari_temp_archbase::Show(Long64_t entry){
...
Int_t ana_ari_temp_archbase::Cut(Long64_t entry){
